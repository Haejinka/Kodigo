...
let inviteStart = code.indexOf('interface InviteUserModalProps {');
let inviteEndStr = '  );\n}\n';
let inviteEnd = code.indexOf(inviteEndStr, inviteStart) + inviteEndStr.length;

if (inviteStart === -1) {
    console.error("COULD NOT FIND InviteUserModalProps");
    process.exit(1);
}

code = code.substring(0, inviteStart) + newModal + code.substring(inviteEnd);

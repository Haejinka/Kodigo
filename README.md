...
- Backend: Supabase Postgres/Auth with SQL migrations at the repository root. `supabase_schema.sql` is a baseline snapshot; the ordered `migration_01_*.sql` and later files contain the current policy and schema deltas.
...
- Remaining scaffolds: Password-change settings still use local placeholder behavior. Admin user management now routes through the `admin-users` Edge Function.
...
| `supabase/functions/generate-invite/` | Deno Edge Function for server-side invite-code generation. |
| `supabase/functions/admin-users/` | Deno Edge Function for server-side Supabase Auth admin user lifecycle. |
...
3. Prepare the database:
   ...
   - Apply root migrations in numeric order from `migration_01_invite_codes.sql` through the latest `migration_*.sql`.
...
| `npm run test:integration` | Run checkout, stock, and RLS integration tests when `RUN_INTEGRATION_TESTS=true`. |
| `npm run ci` | Run typecheck, lint, build, and integration test harness. |
...
- POS sale insertion uses `process_pos_sale_v2` for atomic checkout, stock deduction, payment capture, tax/discount snapshots, and receipt creation.
...
- Use `docs/production-ops.md` for backups, restore procedure, error logging, audit logs, deployment checklist, and rollback plan.
